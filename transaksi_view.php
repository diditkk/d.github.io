...
<!-- content -->
 <div class="row no-guters">
    ...
    <div class="mx-5">
      <h1 class="font-weight-bold mt-1"><i class="fas fa-balance-scale"></i> Data Transaksi</h1>
      <a href="transaksi_tambah.php"><button type="button" class="btn btn-info text-decoration-none float-xl-right mb-3 mr-4"> + Tambah Data</button></a>
      <br>
...
<tr>
 <td><?= $i; ?></td>
  <td><a href="transaksi_ubah.php?id_transaksi=<?= $row["id_transaksi"];?> "><button type="button" class="btn btn-primary btn-sm">Ubah</button></a> 
    <a href="transaksi_hapus.php?id_transaksi=<?= $row["id_transaksi"]; ?>" onclick = "return confirm('Anda yakin untuk menghapus data?')"><button type="button" class="btn btn-danger btn-sm">Hapus</button></a>
  </td>
  <td> <?=$row["no_faktur"];  ?> </td>
  <td> <?=$row["Tgl"];  ?> </td>
